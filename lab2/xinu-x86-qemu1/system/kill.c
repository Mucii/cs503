	...
	)
{
	...
	for(pipid32 i = 0; i<MAXPIPES; i++){
		pipe = &pipe_tables[i];
		//kprintf("reader is %d writer is %d id is %d pid is %d pip state is %d\n",pipe->prevreader,pipe->prevwriter,i,pid,pipe->state);
		if(pipe->owner == pid){
		...
		} else if (pipe->prevreader==pid || pipe->prevwriter == pid){
			//check the state and change it
   			if(pipe->state == PIPE_USED){
   				//this is because we have done clean up in pipdisconnect
   				//cleanup(i)
   				pipe->state = PIPE_FREE;
    			pipe->writer = -1;
    			pipe->reader = -1;
